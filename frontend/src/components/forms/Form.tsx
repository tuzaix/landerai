...
const Form: React.FC<FormProps> = ({ pageId, fields, submitText, onSubmit }) => {
  ...
  return (
    <div className="max-w-md mx-auto p-8 bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100">
      <form onSubmit={handleSubmit} className="space-y-6">
        {fields.map((field, index) => (
          <div key={index}>
            <label htmlFor={field.name} className="block text-sm font-bold text-slate-700 mb-2 ml-1">
              {field.label}
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-transparent rounded-xl focus:bg-white focus:border-blue-500 outline-none transition-all font-medium text-slate-900"
